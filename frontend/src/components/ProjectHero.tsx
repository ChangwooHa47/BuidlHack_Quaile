...
export default function ProjectHero({ name, ticker, phase, description, chains, socials, extraLink }: ProjectHeroProps) {
  const dotColor: Record<Phase, string> = {
    ...
    Contributing: "bg-neon-glow shadow-[0_0_16px_rgba(200,255,0,0.6)]",
    Refunding: "bg-status-settlement shadow-[0_0_16px_rgba(178,166,41,0.6)]",
    Distributing: "bg-status-live shadow-[0_0_16px_rgba(200,255,0,0.6)]",
    Closed: "bg-gray-600",
  };
  const textColor: Record<Phase, string> = {
    Upcoming: "text-status-upcoming",
    Subscribing: "text-neon-glow",
    Contributing: "text-neon-glow",
    Refunding: "text-status-settlement",
    Distributing: "text-status-live",
